...
export default function AI_Prompt({
  ...
  const handleSendClick = () => {
    if (onSubmit && value.trim() && !isLoading) {
      onSubmit();
    }
  };

  return (
    <div className={cn("w-full", compact ? "p-3" : "py-1", className)}>
      <div className={cn(
        "rounded-2xl p-1.5",
        compact
          ? "bg-zinc-900 border border-zinc-800"
          : "bg-black/5 pt-4 dark:bg-white/5"
      )}>
        {showHeader && (
        ...
        <div className="relative">
          <div className="relative flex flex-col">
            <div className="overflow-y-auto" style={{ maxHeight: compact ? "150px" : "400px" }}>
              <Textarea
                className={cn(
                  "w-full resize-none border-none focus-visible:ring-0 focus-visible:ring-offset-0",
                  compact
                    ? "rounded-lg bg-transparent px-3 py-2 text-sm text-zinc-200 placeholder:text-zinc-500 min-h-[40px]"
                    : "rounded-xl rounded-b-none bg-black/5 px-4 py-3 placeholder:text-black/70 dark:bg-white/5 dark:text-white dark:placeholder:text-white/70 min-h-[72px]"
                )}
                id="ai-input-15"
                onChange={(e) => {
            ...
            <div className={cn(
              "flex items-center",
              compact
                ? "h-10 bg-zinc-800/50 rounded-lg mx-1 mb-1"
                : "h-14 rounded-b-xl bg-black/5 dark:bg-white/5"
            )}>
              <div className={cn(
                "flex w-full items-center justify-between",
                compact ? "px-2" : "absolute right-3 bottom-3 left-3 w-[calc(100%-24px)]"
              )}>
                <div className="flex items-center gap-2">
                  ...
                  <div className={cn("mx-0.5 h-4 w-px", compact ? "bg-zinc-700" : "bg-black/10 dark:bg-white/10")} />
                  <label
                    ...
                    className={cn(
                      ...
                      compact
                        ...
                        : "bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-black/40 hover:text-black dark:text-white/40 dark:hover:text-white",
                ...
                >
                  {isLoading ? (
                    <Loader2 className={cn("h-4 w-4 animate-spin", compact ? "text-white" : "dark:text-white")} />
                  ) : (
                    <ArrowRight
                      className={cn(
                        "h-4 w-4 transition-opacity duration-200",
                        compact ? "text-current" : "dark:text-white",
